...
for submission in subreddit.hot(limit = SEARCH_LIMIT):
    ...
    for downloader,patern in downloaders:
        ...
        if not posted.has_key(submission.url) and s.lower().find("r/parallelview") == -1 and re.match(patern,submission.url):
            ...
            while True:
                try:
                    ...
                    print ("posting")
                    title = "[Converted XPost CrossView] " + s + " by u/" + str(submission.author.name)
